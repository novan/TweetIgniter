...
class Tweetigniter {
    ...
    public function __construct() {
        /* Load the config */
        $this->load->config('tweetigniter');
        
        $this->load->helper('url');
        
        $arrConfig = $this->config->item('tweetigniter');
    ...
    protected function _get_access_token() {
        $arrData = $this->_http_request('GET', $this->_access_token_url, array('_cache' => false));
                    ...
                    if(is_array($this->_arrResponses) && array_key_exists($key, $this->_arrResponses)) {
                        ...
                        if($objData->code != 200) {
                            ...
                            if($this->_debug) {
                                ...
                                throw new Tweetigniter_Exception($objData->code.' | Request failed'.$message);
                            }
                        }
    ...
    protected function _normalize_url($url = NULL) {
        ...
        if(!isset($arrUrl['port'])) {
            if(strtolower($arrUrl['scheme']) == 'https') {
                $arrUrl['port'] = 443;
            } else {
                $arrUrl['port'] = 80;
            }
    ...
    protected function _set_access_key($key) {
        ...
        $this->session->set_userdata($this->_token_session, $arrTokens);
    }
    ...
    protected function _update_access($key = null, $value = null) {
        ...
        if(!is_null($key)) {
            $arrSession = $this->session->userdata($this->_token_session);
    ...
    public function get_access_key() {
        ...
        if($access_key === false) {
            ...
            $access_key = null;
            $arrKeys = $this->session->userdata($this->_token_session);
    ...
    public function logout() {
        $this->session->unset_userdata($this->_token_session);
    }
     ...
     * Search
     * 
     * Performs a search of the Twitter system
     * 
     * @param string $query
     * @param array $arrParams
     * @return mixed
     */
    public function search($query, array $arrParams = array()) {
        
        $url = 'search/tweets';
        
        $arrParams['q'] = $query;
        
        $arrResult = $this->call('GET', $url, $arrParams);
...
class Tweetigniter_Exception extends Exception {
